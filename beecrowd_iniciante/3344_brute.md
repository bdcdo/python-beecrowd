...
Seja a função \(f: N \rightarrow N\), que recebe **x** e retorna a quantidade de caracteres do valor escrito por extenso.
...
Dado um inteiro **x**, o resultado da primeira iteração na função f, utilizando **x**, é
                    \(f(x)\), da segunda iteração \(f(f(x))\), da terceira é \(f(f(f(x)))\) e assim por diante. Qual o resultado da 1000ª iteração?
